...
if MODEL_PATH is None:
    raise ValueError(
        "Environment variable `QWEN_MODEL_PATH` is not set. "
        "Please set it, e.g. `export QWEN_MODEL_PATH=/path/to/model`"
    )
...
class QwenPlanner:
    ...
    def _extract_subtasks(self, text) -> list[str]:
        """
        Input: raw text generated by Qwen, typically in this format:
            1. Move the cup to the table.
            2. Pick up the bottle.
            3. Place the bottle into the box.

        This function extracts each numbered subtask and returns:
    ...
    def _prepare_image(self, img) -> Union[Image.Image, str]:
        """
        support inputs:
        - numpy ndarray (H,W,3)
        - PIL.Image
